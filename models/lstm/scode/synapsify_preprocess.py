...
class Preprocess():
    ...
    def _get_rand_sentiment_indices(self, classes, num_indices, forbidden):
        # for cls in forbidden:
        #     del classes[cls]
        forbidden = np.array(forbidden)
        classes = np.array(classes)
        unique = np.unique(classes).tolist()
        num_uniq = len(unique)
        # num_cls_xx = int(math.floor(num_indices / float(num_uniq))) # Assumes there are equal parts of each class
        XX = []
        for this_class in unique:
            this_class_xx = np.where(classes==this_class)[0]
            num_cls_xx = int(math.floor(min(num_indices,len(this_class_xx)) / float(num_uniq)))
            this_class_xx = np.setdiff1d(this_class_xx,forbidden).tolist()
            XX += this_class_xx[0:min(num_cls_xx,len(this_class_xx))]
    ...
    # def _get_class_indices(self,rows, labelcol, forbidden):
    #
    #     # len_init = len(init) # Ruofan fix
    ...
    #         this_class_xx = np.where(class_labels==this_class)[0]
    #         XX[str(this_class)] = np.setdiff1d(this_class_xx,forbidden).tolist()
    #
    #         # this_class_xx = np.where(classes==this_class)[0]
    ...
    # @classmethod
    def _get_rand_indices(self, classes):
    # def _get_rand_indices(self, classes, num_indices, forbidden):
        """
        Function is designed to extract test or training set indices of roughly equal proportions

        :param classes: list of classes for this dataset
        :param num_indices: integer giving how many we want to extract
        :param forbidden: list of indices we can not use
        :return:
        """

        # 1. get the number for each class
        # 2. divide each class into test and train
        classes = np.array(classes)
        unique = np.unique(classes).tolist()
        ttratio = self._train_size / float(self._test_size + self._train_size)
        self._train_xx = []
        self._test_xx  = []
        self._trXX = {}
        self._teXX = {}
        for this_class in unique:
            this_class_xx = np.where(classes==this_class)[0]
            tx = math.floor(len(this_class_xx)*ttratio)
            train_xx = this_class_xx[0:tx].tolist()
            test_xx = this_class_xx[tx:-1].tolist()
            self._train_xx += train_xx
            self._test_xx  += test_xx
            self._trXX[str(this_class)] = train_xx
            self._teXX[str(this_class)] = test_xx
